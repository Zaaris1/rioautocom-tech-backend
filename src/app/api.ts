...
export type MonitoringEventSeverity = "INFO" | "OK" | "WARNING" | "CRITICAL";
export type MonitoringEventCategory = "CONNECTIVITY" | "BACKUP" | "CERTIFICATE" | "SYSTEM";
...
export type MonitoringEvent = {
  ...
  network_id?: string | null;
  network_name?: string | null;
  category: MonitoringEventCategory | string;
  event_type: string;
  severity: MonitoringEventSeverity | string;
  title: string;
  message?: string | null;
...
export type MonitoringHistorySummary = {
  total: number;
  critical: number;
  warning: number;
  ok: number;
...
export async function listMonitoringHistory(params?: {
  store_id?: string;
  category?: string;
  severity?: string;
  days?: number;
  ...
  const q = qs.toString() ? `?${qs.toString()}` : "";
  return apiFetch<{ items: MonitoringEvent[]; summary: MonitoringHistorySummary }>(`/monitoring/history${q}`, { method: "GET" });
}

/* =========================
   Billing / Planos e Mensalidades
========================= */
export type BillingStatus = "TRIAL" | "ATIVO" | "ATRASADO" | "BLOQUEADO" | "CANCELADO" | "SEM_CADASTRO";
...
export async function billingListPlans() {
  return apiFetch<BillingPlan[]>("/billing/plans", { method: "GET" });
}

export async function billingCreatePlan(input: {
  name: string;
  description?: string;
  monthly_price_cents: number;
  max_stores?: number | null;
...
export async function billingUpdatePlan(planId: string, input: Partial<BillingPlan>) {
  return apiFetch<BillingPlan>(`/billing/plans/${planId}`, {
    method: "PATCH",
    body: JSON.stringify(input),
  });
}

export async function billingListSubscriptions() {
  return apiFetch<ClientSubscription[]>("/billing/subscriptions", { method: "GET" });
}

export async function billingUpsertSubscription(clientUserId: string, input: {
  plan_id?: string | null;
  status: string;
  monthly_price_cents?: number | null;
  due_day?: number | null;
  ...
  return apiFetch<ClientSubscription>(`/billing/subscriptions/${clientUserId}`, {
    method: "PUT",
    body: JSON.stringify(input),
  });
}

export async function billingMySubscription() {
  return apiFetch<MySubscription>("/billing/my-subscription", { method: "GET" });
}
