...
type AuditFilter = "all" | "important" | "actions" | "comments";

const AUDIT_FILTERS: { key: AuditFilter; label: string; hint: string }[] = [
  { key: "all", label: "Tudo", hint: "histórico completo" },
  { key: "important", label: "Importantes", hint: "eventos principais" },
  { key: "actions", label: "Ações", hint: "status e atendimento" },
  { key: "comments", label: "Comentários", hint: "mensagens internas" },
];


type CloseChecklistKey = "resolved" | "tested" | "oriented" | "documented" | "evidence";

const CLOSE_CHECKLIST_ITEMS: { key: CloseChecklistKey; label: string; required?: boolean; hint: string }[] = [
  {
    key: "resolved",
    label: "Problema resolvido ou contornado",
    required: true,
    hint: "O atendimento chegou a uma solução prática para a loja.",
  },
  {
    key: "tested",
    label: "Teste final realizado",
    required: true,
    hint: "O sistema, equipamento ou rotina foi validado após o ajuste.",
  },
  {
    key: "oriented",
    label: "Cliente/loja orientado(a)",
    required: true,
    hint: "A loja recebeu a orientação necessária para seguir operando.",
  },
  {
    key: "documented",
    label: "Parecer descreve o que foi feito",
    required: true,
    hint: "O texto explica diagnóstico, ação executada e resultado.",
  },
  {
    key: "evidence",
    label: "Evidência anexada quando necessário",
    hint: "Fotos ou vídeos curtos podem ser anexados em casos que exigem comprovação.",
  },
];

function makeDefaultCloseChecklist(): Record<CloseChecklistKey, boolean> {
  return {
    resolved: false,
    tested: false,
    oriented: false,
...
function buildCloseChecklistText(checks: Record<CloseChecklistKey, boolean>) {
  const lines = CLOSE_CHECKLIST_ITEMS.map((item) => `${checks[item.key] ? "✓" : "—"} ${item.label}`);
  return `Checklist de fechamento:\n${lines.join("\n")}`;
}

function isActionKind(kind: string) {
  const k = String(kind || "").toUpperCase();
  return ["CREATE", "ASSIGN", "START", "PEND", "CLOSE", "REOPEN", "EDIT", "STATUS_CHANGE", "CANCEL"].includes(k);
}
...
export default function TicketDetailPage() {
  ...
  const [closeChecklist, setCloseChecklist] = React.useState<Record<CloseChecklistKey, boolean>>(() => makeDefaultCloseChecklist());
  ...
  const doClose = async () => {
    ...
    const missingRequired = CLOSE_CHECKLIST_ITEMS.filter((item) => item.required && !closeChecklist[item.key]);
    if (missingRequired.length > 0) {
      return show(`Antes de concluir, marque: ${missingRequired.map((item) => item.label).join(", ")}.`, "error");
    }

    const finalParecer = `${parecer.trim()}

${buildCloseChecklistText(closeChecklist)}`;

    setActionLoading("close");
    try {
      const updated = closeFiles.length > 0
        ? await closeTicketWithAttachments(id, finalParecer, closeFiles)
        : await closeTicket(id, finalParecer);
      setTicket(updated);
      show("Chamado concluído!", "success");
      setParecer("");
      setCloseFiles([]);
      setCloseChecklist(makeDefaultCloseChecklist());
      await load();
    } catch (err: any) {
  ...
  const auditTimeline = React.useMemo(() => {
    if (!ticket) return [] as TimelineItem[];
    const items = recentUpdatesAsc.map((u) => {
      const item = buildTimelineItem(u, ticket, derivedAssignedName);
      const rawWho = resolveUpdateActorLabel(u, ticket, derivedAssignedName);
      const who = resolveWhoNice(item.kind, rawWho, ticket, derivedAssignedName);
      return { ...item, who };
    });
    return collapseConsecutiveDuplicates(items);
  }, [recentUpdatesAsc, ticket, derivedAssignedName]);

  const filteredAuditTimeline = React.useMemo(() => {
    return auditTimeline.filter((it) => {
      if (auditFilter === "important") return it.important;
      if (auditFilter === "actions") return isActionKind(it.kind);
      if (auditFilter === "comments") return String(it.kind).toUpperCase() === "COMMENT";
      return true;
    });
  }, [auditTimeline, auditFilter]);

  const visibleAuditTimeline = React.useMemo(() => {
    const base = filteredAuditTimeline.slice().reverse();
    if (showAllHistory) return base;
    return base.slice(0, 20);
  }, [filteredAuditTimeline, showAllHistory]);

  const auditStats = React.useMemo(() => {
    const all = Array.isArray(updatesAsc) ? updatesAsc : [];
    const byKind = (kind: string) => all.filter((u: any) => normalizeKind(u) === kind);
    const firstDate = (...kinds: string[]) => {
      for (const kind of kinds) {
        const found = all.find((u: any) => normalizeKind(u) === kind);
        const when = found?.created_at || found?.createdAt;
    ...
    return {
      total: all.length,
      actions: all.filter((u: any) => isActionKind(normalizeKind(u))).length,
      comments: byKind("COMMENT").length,
      pendencies: byKind("PEND").length,
      attachments: attachments.length,
      openedAt: ticket?.opened_at || firstDate("CREATE"),
      assignedAt: firstDate("ASSIGN"),
      startedAt: firstDate("START"),
      closedAt: firstDate("CLOSE") || (String(ticket?.status || "") === "CONCLUIDO" ? ticket?.updated_at || "" : ""),
      lastAt: last?.created_at || last?.createdAt || ticket?.updated_at || "",
    };
  }, [updatesAsc, attachments.length, ticket]);
  ...
  return (
    <div className="grid">
      <div className="col-12 card">
        {loading || !ticket ? (
          <div className="small">Carregando chamado...</div>
        ) : (
          <>
            {done && (
              <div
                    ...
                    {canCloseNow && (
                      <div className="col-6" style={{ minWidth: 0 }}>
                        <div className="ticket-detail-block ticket-detail-block--success">
                          ...
                          <div className="ticket-close-checklist">
                            <div className="ticket-close-checklist__head">
                              <div>
                                <div className="ticket-close-checklist__title">Checklist de fechamento</div>
                                <div className="small">Padroniza a conclusão e evita chamado fechado sem validação.</div>
                              </div>
                              <span className="badge ok">
                                {CLOSE_CHECKLIST_ITEMS.filter((item) => closeChecklist[item.key]).length}/{CLOSE_CHECKLIST_ITEMS.length}
                              </span>
                            </div>

                            <div className="ticket-close-checklist__items">
                              {CLOSE_CHECKLIST_ITEMS.map((item) => {
                                const checked = !!closeChecklist[item.key];
                                return (
                                  <label
                                    className={"ticket-close-checklist__item " + (checked ? "ticket-close-checklist__item--checked" : "")}
                                    key={item.key}
                                  >
                                    <input
                                      type="checkbox"
                                      ...
                                      disabled={!!actionLoading}
                                      onChange={(e) =>
                                        setCloseChecklist((prev) => ({ ...prev, [item.key]: e.target.checked }))
                                      }
                                    />
                                    <span className="ticket-close-checklist__mark">{checked ? "✓" : ""}</span>
                                    <span className="ticket-close-checklist__body">
      ...
      {!isClient && (
        <div className="col-12 card ticket-audit-panel">
          <div className="row" style={{ justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
            <div>
              <div className="h2" style={{ marginBottom: 2 }}>Auditoria do chamado</div>
              <div className="small" style={{ opacity: 0.9 }}>
                Linha do tempo operacional com responsáveis, horários e principais eventos do atendimento.
                {updatesAsc.length > DETAIL_UPDATES_SOFT_LIMIT ? ` Exibindo os ${DETAIL_UPDATES_SOFT_LIMIT} registros mais recentes.` : ""}
              </div>
            </div>
            {filteredAuditTimeline.length > 20 && (
              <button className="btn" onClick={() => setShowAllHistory((v) => !v)}>
                {showAllHistory ? "Mostrar menos" : `Mostrar tudo (${filteredAuditTimeline.length})`}
              </button>
            )}
          </div>

          <div className="ticket-audit-summary">
            <div className="ticket-audit-summary__card">
              <div className="small">Eventos registrados</div>
              <strong>{auditStats.total}</strong>
              <span>{auditStats.actions} ação(ões) operacionais</span>
            </div>
            <div className="ticket-audit-summary__card">
              <div className="small">Comentários</div>
              <strong>{auditStats.comments}</strong>
              <span>{auditStats.pendencies} pendência(s)</span>
            </div>
            <div className="ticket-audit-summary__card">
              <div className="small">Anexos</div>
              <strong>{auditStats.attachments}</strong>
              <span>{openingAttachments.length} abertura • {closingAttachments.length} fechamento</span>
            </div>
            <div className="ticket-audit-summary__card">
              <div className="small">Última movimentação</div>
              <strong>{auditStats.lastAt ? formatPTBR(auditStats.lastAt) : "—"}</strong>
              <span>status atual: {prettyStatus(ticket?.status)}</span>
            </div>
          </div>

          <div className="ticket-audit-route">
            <div className="ticket-audit-route__step">
              <span>1</span>
              <div>
                <b>Abertura</b>
                <small>{auditStats.openedAt ? formatPTBR(auditStats.openedAt) : "—"}</small>
              </div>
            </div>
            <div className="ticket-audit-route__step">
              <span>2</span>
              <div>
                <b>Atribuição</b>
                <small>{auditStats.assignedAt ? formatPTBR(auditStats.assignedAt) : assignedLabel !== "—" ? assignedLabel : "—"}</small>
              </div>
            </div>
            <div className="ticket-audit-route__step">
              <span>3</span>
              <div>
                <b>Atendimento</b>
                <small>{auditStats.startedAt ? formatPTBR(auditStats.startedAt) : prettyStatus(ticket?.status)}</small>
              </div>
            </div>
            <div className="ticket-audit-route__step">
              <span>4</span>
              <div>
                <b>Conclusão</b>
                <small>{auditStats.closedAt ? formatPTBR(auditStats.closedAt) : "em aberto"}</small>
              </div>
            </div>
          </div>

          <div className="ticket-audit-filters" role="tablist" aria-label="Filtro de histórico do chamado">
            {AUDIT_FILTERS.map((f) => (
              <button
                key={f.key}
                type="button"
                className={"ticket-audit-filter " + (auditFilter === f.key ? "ticket-audit-filter--active" : "")}
          ...
          {visibleAuditTimeline.length === 0 ? (
            <div className="small">Sem atualizações para o filtro selecionado.</div>
          ) : (
            <div className="ticket-timeline ticket-audit-timeline">
              {visibleAuditTimeline.map((it) => (
                <div
                  key={it.id}
                  className={"card ticket-timeline__item " + (it.important ? "ticket-timeline__item--important" : "")}
                >
                  <div className="row" style={{ justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                    <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                      <div className="ticket-audit-icon" aria-hidden="true">{it.icon}</div>
                      <div className="badge" style={{ padding: "6px 10px", borderRadius: 999, ...kindChipStyle(it.kind) }}>
                        {prettyKind(it.kind)}
                      </div>
                      <strong className="wrap-anywhere">{it.title}</strong>
                    </div>
                    <div className="small" style={{ opacity: 0.9 }}>{it.when ? formatPTBR(it.when) : ""}</div>
                  </div>

                  <div className="small wrap-anywhere" style={{ marginTop: 7, opacity: 0.9 }}>
                    Responsável: <b>{it.who || "Sistema"}</b>
                  </div>

                  {it.body && (
                    <div className="ticket-audit-message wrap-anywhere">
                      {String(it.body)}
                    </div>
                  )}
                </div>
