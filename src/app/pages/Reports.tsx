...
export default function ReportsPage() {
  ...
  const executiveHighlights = React.useMemo(() => {
    ...
    if (timeMetrics.closeAvg) {
      ...
      notes.push("Clique em “Calcular tempos” para completar os indicadores de produtividade.");
    }
  ...
  const csvExport = React.useMemo(() => {
    ...
    const rows = ticketsForPeriodInsights.map((t) => {
      const opened = ticketOpenedMs(t);
      const updated = parseDateMs((t as any)?.updated_at);
      const dueMs = opened ? opened + slaHours(t) * 3600 * 1000 : null;
    ...
    return {
      count: rows.length,
      filename: safeFilename(`relatorio-rioautocom-${from}-a-${to}`) + ".csv",
      content: "\ufeff" + csv,
    };
  }, [ticketsForPeriodInsights, from, to]);

  const [csvDownloadUrl, setCsvDownloadUrl] = React.useState("");
  ...
  function handleCsvClick(event: React.MouseEvent<HTMLAnchorElement>) {
    if (loading) {
      event.preventDefault();
      show("Aguarde o carregamento do relatório antes de exportar.", "info");
      return;
    }

    if (!csvExport.count || !csvDownloadUrl) {
      event.preventDefault();
      show("Não há dados no filtro atual para exportar.", "info");
      return;
    }

    show(`CSV gerado com ${csvExport.count} registro(s).`, "success");
  }
      ...
      <div className="card page-hero">
        <div className="page-hero__head">
          <div>
            ...
            <div className="page-hero__sub">
              Indicadores de chamados, SLA, backlog e produtividade.
            </div>
          </div>
          <div className="page-hero__actions report-actions-top">
            ...
            <div className="row no-print" style={{ gap: 8, justifyContent: "flex-end" }}>
              <a
                ...
                role="button"
                aria-disabled={loading || !csvExport.count || !csvDownloadUrl}
        ...
        <div className="page-kpis">
          <div className="page-kpi">
            ...
            <div className="page-kpi__hint">Abertura registrada.</div>
          </div>
          <div className="page-kpi">
          ...
          <div className="page-kpi">
            ...
            <div className="page-kpi__hint">Status atual.</div>
          </div>
          <div className="page-kpi">
            ...
            <div className="page-kpi__hint">Histórico disponível.</div>
          </div>
        </div>
      </div>
      ...
      <div className="card page-section-card filters-sticky">
              ...
              {isAdmin && (
                <div className="col-6">
                  ...
                  <div className="small" style={{ marginTop: 6, opacity: 0.85 }}>
                    * Filtro garantido no frontend (pode ser otimizado via backend depois).
                  </div>
                </div>
              )}
        ...
        <div className="row page-mobile-actions-row" style={{ justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
          <div className="small" style={{ opacity: 0.9 }}>
            Período (abertura): <b>{fmtDateBR(periodFromMs)}</b> até <b>{fmtDateBR(periodToMs)}</b> • Tickets abertos no período:{" "}
        ...
        <div className="page-section-head">
          <div>
            ...
            <div className="page-section-sub">
              Texto pronto para revisão mensal, reunião com cliente ou geração em PDF.
            </div>
          </div>
