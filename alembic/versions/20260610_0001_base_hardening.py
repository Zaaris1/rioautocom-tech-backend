"""base hardening: cnpj_digits, indexes and checks
...
def upgrade():
    ...
    if dialect == "postgresql":
        ...
        # Se houver duplicidade histórica, mantém o primeiro registro com cnpj_digits e zera os repetidos para a migration não quebrar.
        op.execute("""
            WITH ranked AS (
        ...
        op.execute("CREATE INDEX IF NOT EXISTS ix_store_monitoring_last_seen_at ON store_monitoring_status (last_seen_at)")
        op.execute("CREATE INDEX IF NOT EXISTS ix_store_monitoring_reported_status ON store_monitoring_status (reported_status)")
        op.execute("""
        DO $$ BEGIN
        ...
        op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_tickets_type') THEN
                ALTER TABLE tickets ADD CONSTRAINT ck_tickets_type CHECK (type IN ('REPARO', 'SUPORTE', 'VISITA', 'MANUTENCAO', 'OUTRO'));
            END IF;
            ...
            END IF;
            IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ck_tickets_status') THEN
